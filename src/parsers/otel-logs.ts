...
import { type OtlpAttribute, normalizeCodexResourceAttrs } from "./types.js";
	...
	for (const resourceLog of payload.resourceLogs ?? []) {
		...
		for (const scopeLog of resourceLog.scopeLogs ?? []) {
			for (const record of scopeLog.logRecords ?? []) {
				...
				const codexRow = parseCodexOtlpLogRecord(
					record,
					userId,
					resourceJson,
					config,
				...
				// Try OpenCode parser
				const opencodeRow = parseOpencodeOtlpLogRecord(
					record,
					userId,
					resourceJson,
					config,
				...
				const toolName =
					...
					getStringAttr(attrs, "skill.name") ??
					null;
				...
				if (eventName === "claude_code.api_error") {
					logger.warn(
						"Claude API error event session={session} model={model} status={status} error={error} request_id={requestId}",
						{
							...
							model:
								getStringAttr(attrs, "model") ??
								getStringAttr(attrs, "model_name") ??
								"(unknown)",
							status:
								getStringAttr(attrs, "status_code") ??
								getStringAttr(attrs, "status") ??
								"(unknown)",
							...
							requestId:
								getStringAttr(attrs, "request_id") ??
								getStringAttr(attrs, "request.id") ??
								"(missing)",
						},
					);
				}
 ...
 */
export function persistOtlpLogs(
	bodyText: string,
	config?: TelemetryConfig,
): void {
	try {
