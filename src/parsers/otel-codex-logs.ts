...
function attrValue(attribute: OtlpAttribute | undefined): string | undefined {
  return attribute?.value?.stringValue ?? attribute?.value?.intValue?.toString();
}
...
function getCategory(eventName: string, toolName: string | null, eventKind: string | undefined): string | null {
  ...
  if (eventName === 'codex.sse_event') return eventKind === 'response.completed' ? 'sse' : null;
...
export function parseCodexOtlpLogRecord(
  ...
  const timestampNano = toNano(record.timeUnixNano) ?? toNano(getStringAttr(attrs, 'event.timestamp'));
