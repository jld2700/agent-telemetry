...
export async function configCommand(): Promise<void> {
  ...
  if (!existsSync(configPath)) {
    console.log(`${C.yellow}⚠${C.reset} Could not find or create config file at ${configPath}`);
    console.log(`${C.dim}  You can create it manually with your preferred settings.${C.reset}`);
    return;
  }
  ...
  if (result.exitCode !== 0) {
    console.log(`${C.yellow}⚠${C.reset} Editor exited with code ${result.exitCode}`);
  }

  console.log();
  console.log(`${C.green}✓${C.reset} Config file: ${configPath}`);
  console.log(`${C.dim}  Restart the service for changes to take effect:${C.reset}`);
  console.log(`  ${C.bold}agent-telemetry restart${C.reset} ${C.dim}(or reinstall)${C.reset}`);
  console.log();
}