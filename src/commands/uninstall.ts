...
export async function uninstallCommand(
	...
	console.log(
		`${C.bold}${C.red}│  agent-telemetry uninstaller                 │${C.reset}`,
	);
	console.log(
		`${C.bold}${C.red}╰─────────────────────────────────────────────╯${C.reset}`,
	);
	...
	try {
		...
		console.log(
			`  ${C.yellow}⚠${C.reset} ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	if (results.claudeCodeRemoved > 0) {
		console.log(
			`  ${C.green}✓${C.reset} Removed ${results.claudeCodeRemoved} env vars from ~/.claude/settings.json`,
		);
	} else {
	...
	if (results.codexRemoved) {
		console.log(
			`  ${C.green}✓${C.reset} Removed [otel] sections from ~/.codex/config.toml`,
		);
	} else {
		console.log(
			`  ${C.dim}→ No agent-telemetry [otel] sections found${C.reset}`,
		);
	}
	...
	if (results.opencodeRemoved) {
		console.log(
			`  ${C.green}✓${C.reset} Removed OTLP env vars from shell profile`,
		);
	} else {
		console.log(`  ${C.dim}→ No agent-telemetry env vars found${C.reset}`);
	}
	...
	if (opts.purge) {
		...
		console.log(
			`${C.dim}⑥ Data directory preserved (${paths.dataDir})${C.reset}`,
		);
	...
	console.log(
		`  ${C.dim}•${C.reset} Restart Claude Code / Codex / OpenCode for env changes to take effect`,
	);
	if (!opts.purge) {
		console.log(
			`  ${C.dim}•${C.reset} Data directory kept. Run ${C.bold}agent-telemetry uninstall --purge${C.reset} to remove it.`,
		);
	}
	console.log();
}
