...
export async function logsCommand(opts: LogsOptions = {}): Promise<void> {
	...
	if (!stdoutExists && !stderrExists) {
		...
		console.log();
		console.log(
			`${C.dim}The service may not be running or hasn't produced output yet.${C.reset}`,
		);
	...
	const files: string[] = [];
	if ((which === "stdout" || which === "both") && stdoutExists)
		files.push(stdoutLog);
	if ((which === "stderr" || which === "both") && stderrExists)
		files.push(stderrLog);
	...
	console.log(
		`${C.dim}Tailing ${files.length} log file(s)… (Ctrl+C to stop)${C.reset}`,
	);
