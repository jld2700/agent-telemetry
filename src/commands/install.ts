...
import { injectAllOtlp, ensureConfigFile, DEFAULT_OTLP_ENDPOINT } from '../utils/otlp-inject.js';
...
export async function installCommand(opts: InstallOptions = {}): Promise<void> {
  ...
  console.log(`${C.bold}${C.blue}│  agent-telemetry installer                   │${C.reset}`);
  console.log(`${C.bold}${C.blue}╰─────────────────────────────────────────────╯${C.reset}`);
  console.log();
  ...
  if (copied) {
    console.log(`  ${C.green}✓${C.reset} Copied config.yml → ${join(paths.dataDir, 'config.yml')}`);
  } else if (existsSync(join(paths.dataDir, 'config.yml'))) {
    console.log(`  ${C.dim}→ Config already exists: ${join(paths.dataDir, 'config.yml')}${C.reset}`);
  } else {
    console.log(`  ${C.yellow}⚠${C.reset} No config template found, using defaults`);
  }
  ...
  if (!opts.skipService) {
    ...
    console.log(`  ${C.dim}Command: ${cmd.program} ${cmd.args.join(' ')}${C.reset}`);

    try {
      const result = createServiceFile();
      console.log(`  ${C.green}✓${C.reset} Service file: ${result.path}`);
    } catch (err) {
      console.log(`  ${C.red}✗${C.reset} Failed to create service file: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
      ...
      console.log(`  ${C.yellow}⚠${C.reset} Could not start service: ${err instanceof Error ? err.message : String(err)}`);
      console.log(`  ${C.dim}  You can start it manually later.${C.reset}`);
    }
  } else {
    console.log(`${C.dim}③④ Skipping service installation (--skip-service)${C.reset}`);
  }
  ...
  if (!opts.skipOtlp) {
    ...
    if (results.claudeCode.changed) {
      const allKeys = [...results.claudeCode.keysAdded, ...results.claudeCode.keysUpdated];
      console.log(`  ${C.green}✓${C.reset} Updated ${results.claudeCode.path}`);
      console.log(`  ${C.dim}  Keys: ${allKeys.join(', ')}${C.reset}`);
    } else {
      console.log(`  ${C.dim}→ Already up to date: ${results.claudeCode.path}${C.reset}`);
    }
    ...
    if (results.codex) {
      ...
      if (results.codex.changed) {
        ...
        console.log(`  ${C.dim}→ Already up to date: ${results.codex.path}${C.reset}`);
      }
    } else {
  ...
  console.log(`  ${C.dim}•${C.reset} Check status:    ${C.bold}agent-telemetry status${C.reset}`);
  console.log(`  ${C.dim}•${C.reset} View logs:       ${C.bold}agent-telemetry logs${C.reset}`);
  console.log(`  ${C.dim}•${C.reset} Edit config:     ${C.bold}agent-telemetry config${C.reset}`);
  console.log(`  ${C.dim}•${C.reset} Restart Claude Code for OTLP env vars to take effect`);
  console.log();
  console.log(`${C.bold}How to verify it's working:${C.reset}`);
  console.log(`  ${C.dim}•${C.reset} Run any Claude Code session, then check:`);
  console.log(`    ${C.bold}agent-telemetry status${C.reset} ${C.dim}(should show event count > 0)${C.reset}`);
  console.log();
