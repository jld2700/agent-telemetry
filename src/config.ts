...
function mergeConfig(raw: Record<string, unknown>): TelemetryConfig {
	...
	const config: TelemetryConfig = {
		data_dir:
			typeof raw.data_dir === "string"
				? expandTilde(raw.data_dir)
				: DEFAULT_CONFIG.data_dir,
		server: {
			host:
				typeof serverObj.host === "string"
					? serverObj.host
					: DEFAULT_CONFIG.server.host,
			port:
				typeof serverObj.port === "number"
					? serverObj.port
					: DEFAULT_CONFIG.server.port,
		},
		collect_logs:
			typeof raw.collect_logs === "boolean"
				? raw.collect_logs
				: DEFAULT_CONFIG.collect_logs,
		collect_metrics:
			typeof raw.collect_metrics === "boolean"
				? raw.collect_metrics
				: DEFAULT_CONFIG.collect_metrics,
		collect_traces:
			typeof raw.collect_traces === "boolean"
				? raw.collect_traces
				: DEFAULT_CONFIG.collect_traces,
	...
	if (env.AGENT_TELEMETRY_DATA_DIR)
		config.data_dir = expandTilde(env.AGENT_TELEMETRY_DATA_DIR);
			...
			if (filterRaw && typeof filterRaw === "object") {
				...
				agentCfg[signal] = {
					...
					list: Array.isArray(f.list)
						? f.list.filter((x) => typeof x === "string")
						: [],
				};
			}
		}
...
function parseUpstream(raw: unknown): UpstreamConfig {
	...
	return {
		...
		batch_size:
			typeof u.batch_size === "number"
				? u.batch_size
				: DEFAULT_CONFIG.upstream.batch_size,
		interval_ms:
			typeof u.interval_ms === "number"
				? u.interval_ms
				: DEFAULT_CONFIG.upstream.interval_ms,
		report_logs: typeof u.report_logs === "boolean" ? u.report_logs : true,
		report_metrics:
			typeof u.report_metrics === "boolean" ? u.report_metrics : true,
	};
}
...
function parseAuth(raw: unknown): AuthConfig {
	...
	const type =
		a.type === "basic" || a.type === "bearer" || a.type === "header"
			? a.type
			: "none";
