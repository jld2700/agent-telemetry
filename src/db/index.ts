...
export function getPendingLogEvents(limit: number) {
	return getDb()
		.query(
			"SELECT * FROM log_events WHERE uploaded_at IS NULL ORDER BY id LIMIT ?",
		)
...
export function markLogEventsUploaded(ids: number[]): void {
	...
	d.transaction(() => {
		const stmt = d.prepare(
			"UPDATE log_events SET uploaded_at = ? WHERE id = ?",
		);
...
export function getPendingOtelMetrics(limit: number) {
	return getDb()
		.query(
			"SELECT * FROM otel_metrics WHERE uploaded_at IS NULL ORDER BY id LIMIT ?",
		)
		.all(limit) as Array<Record<string, unknown>>;
}
...
export function markOtelMetricsUploaded(ids: number[]): void {
	...
	d.transaction(() => {
		const stmt = d.prepare(
			"UPDATE otel_metrics SET uploaded_at = ? WHERE id = ?",
		);
		for (const id of ids) stmt.run(now, id);
	})();
}
