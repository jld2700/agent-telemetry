...
export async function runLogEventsReport(config: TelemetryConfig): Promise<void> {
  if (isRunning) {
    logger.warn('Reporter log-events: previous run still in progress, skipping this cycle');
    return;
  }
  isRunning = true;
  try {
...
function parseAttributes(id: number, raw: string): Record<string, unknown> {
  try {
    ...
    logger.warn('Reporter log-events: corrupt attributes JSON on record {id}: {error}, sending empty object', {
      id,
...
async function uploadLogEventsBatch(url: string, payload: { records: unknown[] }, ids: number[]): Promise<void> {
  ...
  for (let attempt = 1; attempt <= LOG_EVENTS_MAX_RETRIES; attempt++) {
    try {
      ...
      if (res.status === 429) {
        ...
        logger.warn('Reporter log-events: rate-limited (429), retry {attempt}/{maxRetries}', {
          attempt,
        ...
        logger.warn('Reporter log-events: {status} from server, retry {attempt}/{maxRetries}', {
          status: res.status,
          attempt,
          maxRetries: LOG_EVENTS_MAX_RETRIES,
        });
      ...
      logger.warn('Reporter log-events: network error (attempt {attempt}/{maxRetries}): {message}', {
        attempt,
        maxRetries: LOG_EVENTS_MAX_RETRIES,
  ...
  logger.warn('Reporter log-events: all retries exhausted, batch will be retried next cycle');
}
