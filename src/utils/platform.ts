...
function generateSystemdService(
	...
	const execStart =
		`${cmd.program} ${cmd.args.map(escapeShell).join(" ")}`.trim();
	return `[Unit]
...
export function createServiceFile(): { path: string; created: boolean } {
	...
	const content =
		platform === "macos"
			? generatePlist(paths, cmd)
			: generateSystemdService(paths, cmd);
...
function startLaunchdService(paths: ServicePaths): {
	started: boolean;
	message: string;
} {
	...
	if (
		stderr.includes("already bootstrapped") ||
		stderr.includes("Bootstrap failed: 5")
	) {
		...
		if (ks2.exitCode === 0) {
			return {
				started: true,
				message: "Service started (kickstart after bootstrap)",
			};
		}
	}
...
function startSystemdService(_paths: ServicePaths): {
	started: boolean;
	message: string;
} {
	...
	if (enableResult.exitCode === 0) {
		return {
			started: true,
			message: "Service started (systemctl enable --now)",
		};
	}
...
function stopLaunchdService(paths: ServicePaths): {
	stopped: boolean;
	message: string;
} {
	const uid = process.getuid?.() ?? 501;
	...
	if (
		stderr.includes("not bootstrapped") ||
...
function stopSystemdService(_paths: ServicePaths): {
	stopped: boolean;
	message: string;
} {
